...
def lambda_handler(event, context):
    ...
    if len(myresult1) == 0:
        ...
        statusCode = 200
        message = "Requested user id do not have any trip  record"
        
        mycursor.execute(query2)
        myresult2 = mycursor.fetchall()
        if len(myresult2) > 0:
            statusCode = 201
            response['user_rating'] = float(myresult2[0][0])
            response['user_ride_count'] = myresult2[0][1]
            message = "Successfully retrieved results for requested user id"
        
        response['user_name'] = myresult1[0][0]
        response['user_cnic'] = myresult1[0][1]
        response['user_email'] = myresult1[0][2]
        response['user_dob'] = str(myresult1[0][3])
        response['user_picture'] = myresult1[0][4]
